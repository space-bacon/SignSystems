...
def split_text_into_batches(text, max_tokens):
    ...
    for word in words:
        if len(" ".join(current_batch + [word])) > max_tokens:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            for image in page.images:
                im = page.within_bbox((image['x0'], image['top'], image['x1'], image['bottom'])).to_image()
                image_path = f"image_page{page.page_number}.png"
                im.save(image_path)
...
def main():
    input_files = [f for f in os.listdir(".") if f.endswith((".pdf", ".txt", ".png", ".jpg", ".jpeg"))]
    keyword = ""
    images = []

    for input_file in input_files:
        if input_file.endswith(".pdf"):
            pdf_keyword, pdf_images = extract_text_and_images_from_pdf(input_file)
            keyword += pdf_keyword + " "
            ...
            keyword += extract_text_from_image(input_file) + " "
